...
static void popup_surface_frame_callback(void *data, struct wl_callback *cb,
                                         uint32_t time) {
    wl_callback_destroy(cb);
...
static void draw(void) {
    ...
    if (animate) {
        ...
        if (old_top != margin_top) {
            zwlr_layer_surface_v1_set_margin(layer_surface, margin_top, 0, 0,
                                             0);
...
static void draw_popup(void) {
    ...
    eglMakeCurrent(egl_display, popup_egl_surface, popup_egl_surface,
                   egl_context);
...
static void wl_pointer_enter(void *data, struct wl_pointer *wl_pointer,
    ...
    wl_pointer_set_cursor(wl_pointer, serial, cursor_surface, image->hotspot_x,
                          image->hotspot_y);
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "k:nw:h:o:l:a:x:m:t:")) != -1) {
        switch (c) {
            ...
            case 'l': {
                ...
                found = false;
                for (size_t i = 0; i < sizeof(layers) / sizeof(layers[0]);
                     ++i) {
            ...
            case 'a': {
                ...
                found = false;
                for (size_t i = 0; i < sizeof(anchors) / sizeof(anchors[0]);
                     ++i) {
            ...
            case 'k': {
                ...
                found = false;
                for (size_t i = 0; i < sizeof(kb_int) / sizeof(kb_int[0]);
                     ++i) {
                ...
                if (!found) {
                    fprintf(stderr,
                            "invalid keyboard interactivity setting %s\n",
                            optarg);
                    return 1;
                }
                break;
    ...
    assert(egl_window);
    egl_surface = eglCreatePlatformWindowSurfaceEXT(egl_display, egl_config,
                                                    egl_window, NULL);
